...
export function spotify_api(): SpotifyApi {
	if (_spotify_api) {
		return _spotify_api
	}

	const [[client_id, client_secret], ] = cred_get('spotify')

	_spotify_api = SpotifyApi.withClientCredentials(client_id, client_secret, scopes, {
		// i tried, hamfisting locale into requests doesn't work
...
export function zotify_credentials(): [string, string] {
	return cred_get('spotify_dl_user')[0]
}
